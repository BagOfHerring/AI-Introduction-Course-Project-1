...
CallbackReturn MultiModeController::on_init() {
  try {
    auto declare_string_array = [this](const std::string& name) {
    ...
    auto declare_string = [this](const std::string& name, const std::string& default_value) {
      if (!get_node()->has_parameter(name)) {
        auto_declare<std::string>(name, default_value);
      }
    };

    if (!get_node()->has_parameter("arm_count")) {
      auto_declare<int>("arm_count", 1);
    }
    declare_string_array("controllers");
    declare_string_array("start_controllers.names");

    num_robots = get_node()->get_parameter("arm_count").as_int();
    if (num_robots < 1) {
      fprintf(stderr, "arm_count must be at least 1.\n");
      return CallbackReturn::ERROR;
    }

    for (int i = 1; i <= num_robots; ++i) {
      declare_string("arm_" + std::to_string(i) + ".arm_id", "panda");
    }

    auto controllers = get_node()->get_parameter("controllers").as_string_array();
    for (const auto& controller : controllers) {
      declare_string_array("resources." + controller);
      declare_string_array("start_controllers.resources." + controller);
    }

    auto start_controllers = get_node()->get_parameter("start_controllers.names").as_string_array();
